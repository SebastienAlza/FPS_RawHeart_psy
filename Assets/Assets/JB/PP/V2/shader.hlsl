...
float3 NonLocalMeans(float2 uv, int kernelSize, float sigma)
{
...
float3 DynamicBloom(float3 color)
{
...
float2 ColorBasedDistortion(float2 uv, float3 color)
{
...
void main_float(float2 uv, float kernelSize, float sigma, out float3 Result)
{
    ...
    float3 nlmColor = NonLocalMeans(uv, kernelSize, sigma);

    // Calcul de la courbure locale pour ajuster la saturation
    float curvature = Curvature(uv);
    ...
    float3 bloomedColor = DynamicBloom(perlinColor);

    // Application de la distorsion par couleur
    float2 distortedUV = ColorBasedDistortion(uv, bloomedColor);
